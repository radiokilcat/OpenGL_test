...
#
args = get_args()
execute(['conan', 'profile', 'new', '--detect', 'alex_profile'], raise_exception=False)
if os_windows():
    execute(['conan', 'profile', 'update', 'settings.compiler.runtime=MT', 'alex_profile'])
else:
    execute(['conan', 'profile', 'update', 'settings.compiler.libcxx=libstdc++11', 'alex_profile'])

execute_in(output_path=os.path.join(os_current_file_path(), 'build', args.build_type),
